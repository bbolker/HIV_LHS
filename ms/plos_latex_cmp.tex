...
%DIF DEL plos_latex_old.txt   Fri Oct 14 10:07:48 2016
%DIF ADD plos_latex.tex       Tue Oct 18 14:17:21 2016
% Template for PLoS
...
course of an emerging epidemic have important implications \DIFdelbegin \DIFdel{both for
our basic }\DIFdelend \DIFaddbegin \DIFadd{for
our }\DIFaddend understanding of epidemiological dynamics\DIFdelbegin \DIFdel{and,
  potentially, for the outcomes of public health interventions.
Changes }\DIFdelend \DIFaddbegin \DIFadd{.
While changes }\DIFaddend in the fitness landscape
\DIFdelbegin \DIFdel{will }\DIFdelend generally select for higher virulence \DIFdelbegin \DIFdel{during the early phase of }\DIFdelend \DIFaddbegin \DIFadd{early in }\DIFaddend an
epidemic, \DIFdelbegin \DIFdel{but quantitative outcomes
can }\DIFdelend \DIFaddbegin \DIFadd{quantitative outcomes
may }\DIFaddend depend sensitively on biological details and the structure of
...
complexity predicted slower progression (a minimum of 15 years to
...
importance. \DIFaddbegin \DIFadd{In general, evolutionary theory suggests that
disease strains that can reproduce more --- where reproduction
is defined the amount of }\emph{\DIFadd{between-host}} \DIFadd{transmission, or
the number of new hosts infected --- will increase in prevalence.
Pathogens can increase their net reproduction rate either
by increasing their birth rate, the rate (per infected host) at which they
infect new hosts, or by decreasing their death rate, the rate
at which hosts recover or die from disease.
}\DIFaddend The trade-off theory \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Ebert1999} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{anderson_coevolution_1982,Ebert1999} }%DIFAUXCMD
}\DIFaddend --- \DIFdelbegin \DIFdel{which
postulates that
parasite virulence can be explained as the long-term
evolutionary outcome of a saturating relationship between parasite
clearance rate and transmission
rate --- has been criticized }\DIFdelend \DIFaddbegin \DIFadd{postulates that
the birth and death rate are both linked to the rate at which
the pathogen reproduces within the host, and that the pathogen
...
The uncoupled and extra-couple transmission rates are scaled by multiplying the within-couple transmission rate $\beta$ by the contact ratios $c_u/c_w$ and $c_e/c_w$ \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(see Appendix). Simplifying the model of HIV pathogenesis from three stages to a single stage could affect our conclusions about the evolution of virulence (e.g. Kretzschmar and Dietz \mbox{%DIFAUXCMD
\cite{kretzschmar_effect_1998} }%DIFAUXCMD
show that pair formation dynamics and multiple stages of infectivity have interactive effects on $\rzero$). However, our simplified model produces results that are qualitatively similar to those of Shirreff \etal's \mbox{%DIFAUXCMD
\cite{shirreff_transmission_2011} }%DIFAUXCMD
model; when our model is calibrated to have a similar initial epidemic growth rate $r$, the peak }\Lspvl\DIFadd{\ occurs at the same time ($\approx$ 200 years) but slightly higher (4.7 vs. 4.3); the relative height of the peak is \todo{compute difference and fill this in}
(\figurename~\ref{fig:panel3}).
}\DIFaddend 

\DIFaddbegin \begin{figure}[!ht]
\includegraphics[width=\textwidth]{../figures/fig1.pdf}
\caption{{\bf \DIFaddFL{Baseline dynamics.}}
\DIFaddFL{Time series of mean population }\Lspvl\DIFaddFL{. (a) Contrast with three-stage Shirreff model and effects of varying exponential growth rate, $r$. (b) Effects of varying initial infectious density $I(0)$. (c) Effects of varying initial mean virulence $\alpha(0)$. The orange curve in panel (a) shows that our simplified model can produce similar trajectories to those Shirreff \etal's model \mbox{%DIFAUXCMD
\cite{shirreff_transmission_2011}}%DIFAUXCMD
. In order to compare the models, we match the initial exponential growth rate (see main text ``Simulation runs"). Furthermore, we experiment with wide ranges of initial values (panels b and c) to show that evolutionary dynamics are not overly sensitive to starting parameter values, which we hold constant in our simulations.}}
\label{fig:panel3}
\end{figure}
...
\DIFaddend The next two models\DIFaddbegin \DIFadd{, which are intended to bridge the gap between models with fully explicit pair-formation dynamics and simpler, implicit models, }\DIFaddend assume instantaneous partnership formation (``instswitch'')\DIFdelbegin \DIFdel{and thus consist of }\DIFdelend \DIFaddbegin \DIFadd{. The compartmental structure thus omits the single states $S$ and $I$, comprising }\DIFaddend only the three partnered states: $SS$, $SI$, and $II$. Like the first two models, \DIFdelbegin \DIFdel{these models differ }\DIFdelend \DIFaddbegin \DIFadd{this pair of models differs }\DIFaddend in their inclusion of extra-pair contact: the third model (``instswitch+epc'') includes extra-partnership contact (now only with individuals in other partnerships, since uncoupled individuals \DIFdelbegin \DIFdel{don't }\DIFdelend \DIFaddbegin \DIFadd{do not }\DIFaddend exist in this model) \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{while }\DIFaddend the fourth (``instswitch'') only considers within-couple transmission.

The \DIFdelbegin \DIFdel{last two models do not explicitly track sexual partnerships}\DIFdelend \DIFaddbegin \DIFadd{next two models represent extreme simplifications of sexual partnership dynamics}\DIFaddend .  One (``implicit'') is an implicit serial monogamy model based on the epidemiological model used by Shirreff \etal\ \cite{shirreff_transmission_2011}. It is actually a random mixing model that consist of only two states, $S$ and $I$, and does not consider explicit partnership dynamics. However, to reflect the effect of \DIFdelbegin \DIFdel{(instantaneously formed) }\DIFdelend partnership structure, it uses an adjusted transmission rate that is derived from an approximation of the basic reproduction number of a serial monogamy model \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{\todo{add something about choice of heterogeneity}
}
...
\DIFaddbegin \DIFadd{Although each disease strain's core characteristic is its SPVL, the
...
SPVL (measured in }\Lspvl\DIFadd{\ units), we summarize the virulence
...
}\DIFaddend For each model we derive the following summary statistics:
\DIFdelbegin \DIFdel{peak virulence, peak time , equilibrium virulence, and relative peak virulence . The transient phase of an epidemic is often characterized by high virulence, and we define peak virulence as the maximum virulence during this phase. It is simply calculated by taking the maximum value from the
virulence trajectory, and peak time is the time at which the maximum valueis reached. Once the epidemic enters the endemic phase, evolution of virulence stabilizes and reaches equilibrium. Equilibrium virulence is calculated by taking the mean virulence at }\DIFdelend \DIFaddbegin \DIFadd{minimum expected time to progression;
...
(Figure~\ref{fig:panel3}c) to mean that, in the absence of
...
Across the entire range of parameters covered by the LHS analysis, all of the classes of models we considered produce qualitatively similar virulence trajectories (\figurename~\ref{fig:virtraj}). Although the speed of virulence evolution varies, leading to wide variation in the \DIFdelbegin \DIFdel{peak virulence }\DIFdelend \DIFaddbegin \DIFadd{minimum progression time }\DIFaddend (means ranging from approximately \DIFdelbegin \DIFdel{3.75 to 4.5 }%DIFDELCMD < \Lspvl%%%
\DIFdelend \DIFaddbegin \DIFadd{\todo{fill in}}\DIFaddend ), virulence peaks in all models between 200 and 300 years.
...
Our chosen summary statistics (peak time, \DIFdelbegin \DIFdel{peak virulence, equilibrium
virulence}\DIFdelend \DIFaddbegin \DIFadd{minimum progression time, equilibrium
...
(\figurename~\ref{fig:pairplot}, leftmost column) because the equilibrium virulence is low}\DIFdelend .
...
Single individuals acquire partners at per-person rate $\rho$. Partnership formation rate for $S$, $I_i$ are \DIFdelbegin \DIFdel{$- \rho S$ and $-\rho I_i$}\DIFdelend \DIFaddbegin \DIFadd{$\rho S$ and $\rho I_i$}\DIFaddend , respectively. We follow Champredon \etal's results \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
Champredon \etal\ \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{champredon_hiv_2013} }%DIFAUXCMD
}\DIFaddend define the proportion of infectious extra-couple and uncoupled contact through the following term:
...
Single susceptible individuals become infected \DIFaddbegin \DIFadd{through uncoupled contact at per-person rate $\sum_k P_k U_k$ }\DIFaddend and enter single infected \DIFdelbegin \DIFdel{compartment at the total rate of $\sum_k P_k U_k S$}\DIFdelend \DIFaddbegin \DIFadd{state}\DIFaddend . Through mutation, newly infected individuals are distributed into single infected compartments with different strains: $\ET(I_i) = \sum_k M_{ki} P_k U_k S$. Either partner in an $SS$ partnership \DIFdelbegin \DIFdel{can be infected , with the partnership state becoming }\DIFdelend \DIFaddbegin \DIFadd{becomes infected at per-person rate $\sum_k P_k E_k$, and partnership state changes to an }\DIFaddend $SI$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{partnership }\DIFaddend at the total rate of $\sum_i 2 P_i E_i SS$. The formation of $SI_i$ partnerships is similar to the process through which single susceptible individuals are distributed into single infected compartments: $\ET(SI_i) = \sum_k 2 M_{ki} P_k E_k SS$. Lastly, the susceptible partner in an $SI$ partnership \DIFdelbegin \DIFdel{can be infected due to uncoupled/}\DIFdelend \DIFaddbegin \DIFadd{becomes infected from }\DIFaddend extra-couple contacts \DIFdelbegin \DIFdel{and partnership can change }\DIFdelend \DIFaddbegin \DIFadd{at a per-person rate of $\sum_k P_k E_k$, and partnership state changes }\DIFaddend to an $II$ partnership. As in the previous cases, $SI_i$ partnerships are lost at a rate of $\sum_k P_k E_k SI_i$. The mutation process is similar to that of within-couple transmission. The only difference is that the \Lspvl\ of a newly infected partner is not determined by its social partner but from an extra-couple partner (i.e. the term $P_i$): $\ET(II_{ij}) = (\frac{1}{2})^{\delta_{ij}}(\sum_k (M_{kj} P_k E_k SI_i + M_{ki} P_k E_k SI_j))$. Combining these equations we get the following set of equations that describe all the transmission dynamics:
...
If an infected individual in a partnership dies, the partnership dissolves. Thus, an $SI_i$ partnership dissolves at \DIFdelbegin \DIFdel{a rate $-\lambda_i$}\DIFdelend \DIFaddbegin \DIFadd{per-partnership rate $\lambda_i$}\DIFaddend , and the susceptible partner enters the single susceptible compartment at rate $\lambda_i SI_i$ (due to the SIS formulation\DIFdelbegin \DIFdel{the the }\DIFdelend \DIFaddbegin \DIFadd{, the }\DIFaddend infected partner that dies also gives rise, at an equal rate, to an individual entering the single susceptible compartment):
...
Since model 3 and 4 assume instantaneous partnership formation, there are only three states: $SS$, $SI_i$, and $II_{ij}$. Partnership dissolution rates are equal to those of model 1 and 2: $\DS(SS) = -cSS$, $\DS(SI_i) = - cSI_i$, and \DIFdelbegin \DIFdel{$\DS(II_{ij}) = - II_{ij}$}\DIFdelend \DIFaddbegin \DIFadd{$\DS(II_{ij}) = - c II_{ij}$}\DIFaddend . Once individuals leave a partnership, they \DIFdelbegin \DIFdel{enter temporary compartments and are distributed into a partnership as follows}\DIFdelend \DIFaddbegin \DIFadd{are instantaneously distributed into coupled states. In order to make the equations simpler, we introduce the following two terms: $X$ and $Y_i$, where $X$ denotes the number of susceptible individuals that leave the partnership at a given time, and $Y_i$ the number of infected individuals with }\Lspvl\DIFadd{\ of $\alpha_i$ who leave partnership at a given time. These temporarily single individuals then form couples through the same partnership formation rule described in the previous section}\DIFaddend :
...
%DIFDELCMD < X =& 2 c SS + \sum_k c SI_k + \sum_k 2 \lambda_k SI_k + \sum_l \sum_k 2^{\delta_{lk}}  \lambda_k II_{lk},\\
%DIFDELCMD < Y_i =& c SI_i + \sum_k 2^{\delta_{ik}}  c II_{ik} + \sum_k  2^{\delta_{ik}}  \lambda_k II_{ik}, \\
%DIFDELCMD < SS  =& - c SS + \frac{X^2}{2 (X + \sum_k Y_k)}  - \sum_i 2 P_i E_i SS,\\
%DIFDELCMD < SI_i =& - c SI_i + \frac{X Y_i}{X + \sum_k Y_k} - \beta_i SI_i + \sum_k 2 M_{ki} P_k E_k SS\\
%DIFDELCMD < &- \sum_k P_k E_k SI_i - \lambda_i SI_i,\\
%DIFDELCMD < II_{ij}& - c II_{ij} +\khalf \frac{Y_i Y_j}{X + \sum_k Y_k} + \khalf \cdot (M_{ij} \beta_i SI_i + M_{ji} \beta_j SI_j)\\
%DIFDELCMD < &+ \khalf \cdot (\sum_k (M_{kj} P_k E_k SI_i + M_{ki} P_k E_k SI_j)) -(\lambda_i + \lambda_j) II_{ij}.
...
\begin{equation}
\beta\DIFdelbegin \DIFdel{'}\DIFdelend \DIFaddbegin \DIFadd{^\ast}\DIFaddend _i = \frac{c \beta_i}{c + \beta_i + \lambda_i}.
\end{equation}
Thus, we get the following model:

\begin{equation}
\DIFdelbegin %DIFDELCMD < \begin{aligned}
%DIFDELCMD < S' & = \sum_k \lambda_k I_k - \sum_k \beta'_k S I_k.\\
%DIFDELCMD < I_i & = \sum_k M_{ki} \beta'_k S I_k - \lambda_i I_i.
%DIFDELCMD < \end{aligned}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{aligned}
...
\begin{equation}
\DIFdelbegin %DIFDELCMD < \begin{aligned}
%DIFDELCMD < S' & = \sum_k \lambda_k I_k - \sum_k \beta_k S I_k,\\
%DIFDELCMD < I_i & = \sum_k M_{ki} \beta_k S I_k - \lambda_i I_i.
%DIFDELCMD < \end{aligned}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{aligned}
...
\DIFaddbegin \subsection*{\DIFadd{Heteregenous model}}

\DIFadd{We extend "pairform+epc model" by allowing for heterogeneity in sexual behaviour. Since pairform+epc model captures four distinct sexual behaviours -- pair formation, pair dissolution, extra-couple mixing, and uncoupled mixing -- we assume that all four parameters that model the mentioned behaviours are scaled by the same factor based on the risk group. In other words, an individual in a higher risk is more likely to form a stable partnership, leave a stable partnership, and engage in a extra-couple/uncoupled mixing. We denote this scaling parameter as $\varphi_i$ where $i$ is the risk group. For simplicity, we assume that the transmission rate per partnership is not affected by sexual behaviour.
}

\subsubsection*{\DIFadd{Partnership dynamics}}

\DIFadd{Individuals in a risk group $i$ leave single state at per-person rate $\varphi_i \rho$. Let $XY_{ij,kl}$ be a coupled state where $X$ and $Y$ are the infection status (susceptible or infected) of each partner, $k$ and $l$ are the risk groups $X$ and $Y$ belong to respectively, and $i$ and $j$ are the strains of an infected partner. If a partner is susceptible, strain index is replaced by $\cdot$. For example, $SI_{\cdot j,kl}$ is the number of partners where the susceptible partner is in risk group $k$ and infected partner is in risk group $l$ and has }\Lspvl\DIFadd{\ of $\alpha_j$. For simplicity, we assume that people mix randomly. Then, we can write the partnership formation process as follows:
}

\begin{equation}
\DIFadd{\PF(XY_{ij, kl}) = \kkhalf \frac{\varphi_k \rho X_{i,k} \varphi_l \rho Y_{j, l}}{\sum\limits_m \varphi_m \rho (S_{\cdot, m} + \sum\limits_n I_{n,m})}
}\end{equation}

\DIFadd{For dissolution process, individual in risk group $i$ expects to leave partnership at a rate $\varphi_i c$. If partnership is formed between two individuals from a different risk group, rate at which they leave the partnership differs. We resolve this conflict by assuming that a partnership dissolution rate of a couple is equal to the average of that of two partners. Therefore, $XY_{ij, kl}$ dissolve at per-partnership rate $\frac{\varphi_k + \varphi_l}{2} c$, and both $X_{i,k}$ and $Y_{j,l}$ partners return to single state at the same rate.
}

\subsubsection*{\DIFadd{Heterogeneous models: Infection dynamics}}

\DIFadd{Since we assume that the rate of transmission per partnership stays constant across different risk groups, within-couple infection process is similar to other models:
}
...
\DIFadd{We now define $P_{k,i}$ as the proportion of the extra-couple and uncoupled contact that arises from an infected individual from risk group $i$ with }\Lspvl\DIFadd{\ of $\alpha_k$:
}\begin{equation}
\DIFadd{P_{k,i} = \frac{\varphi_i r_u I_{k,i} + \varphi_i r_e (SI_{k,i} + \sum_j \sum_l 2^{\delta_{kl} \delta_{ij} } II_{kl,ij} )}{\sum_j Q_j}
}\end{equation}
\DIFadd{Since the relative uncoupled/extra couple contact ratio are scaled by the factor of $\varphi_i$, uncoupled and extra-couple transmission rates are scaled by the same factor as well: $U_{k,i} = \varphi_i r_u \beta_k$ and $E_{k,i} = \varphi_i r_e \beta_k$. Once again, we assume random mixing between individuals. Then, a susceptible individual in risk group $i$ becomes infected through extra-couple and uncoupled contact at a per capita rate of $\sum_j \sum_k P_{k,j} X_{k,i}$. Once infected, individuals are distributed into each Strain categories through mutation.
}

\subsubsection*{\DIFadd{Heterogeneous model: Disease induced mortality}}

\DIFadd{Disease induced mortality is not affected by the sexual behaviour of an individual. 
}

\DIFaddend \subsection*{Initial distribution of infected individuals}
...
\end{aligned}
\end{equation}
\DIFdelbegin \DIFdel{Lastly, as }\DIFdelend \DIFaddbegin \DIFadd{As }\DIFaddend model 5 is an implicit model, which does not consider different stages of partnership, we have the following initial distribution:
...
\DIFadd{For this particular simulation, we disregard infection process as well as disease induced mortality in order to preserve the strain distribution of infected individuals. Furthermore, since the scaling parameter, $\gamma$, does not affect the risk group distribution in the absence of disease transmission, we increase the scaling parameter to 5 ($\gamma = 5$) to speed up the simulation and run the model for 50 years. After the model has reached its quasi-equilibrium state, we take this distribution of susceptible and infected individuals as the initial state of the actual simulation.
}

\DIFaddend \paragraph*{Appendix S2: dynamics of transmission and virulence}
...
\DIFdel{HIV}%DIFDELCMD < } %%%
\DIFdel{Virulence and Community Viral Load. }%DIFDELCMD < \newblock {%%%
...
%DIFDELCMD < %%%
\DIFdel{Day T, Proulx SR. }%DIFDELCMD < \newblock %%%
...
%DIFDELCMD < %%%
\DIFdel{Ma J, Dushoff J, Bolker BM, Earn DJD.}%DIFDELCMD < \newblock %%%
\DIFdel{Estimating Initial Epidemic Growth Rates. }%DIFDELCMD < \newblock %%%
...
%DIFDELCMD < \bibitem{herbeck2016evolution}
%DIFDELCMD < %%%
...
\DIFdel{A}%DIFDELCMD < } %%%
\DIFdel{meta-analysis of trends in prognostic markers of }%DIFDELCMD < {%%%
\DIFdel{HIV}%DIFDELCMD < } %%%
...
\DIFdel{Proceedings of the National Academy of Sciences.
  2014;111(50):E5393--E5400. }%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
