...
%DIF ADD plos_latex.tex       Tue Oct 25 13:45:11 2016
% Template for PLoS
...
\end{equation}
Movement from $SS$ state to $SI$ state and $SI$ to $SS$ is modeled through the same \DIFdelbegin \DIFdel{equation that is used in model }\DIFdelend \DIFaddbegin \DIFadd{equations that are used in models }\DIFaddend 1 and 2.

\subsubsection*{Instantaneous-switching models: Disease induced mortality}

\DIFdelbegin \DIFdel{Disease induced }\DIFdelend \DIFaddbegin \DIFadd{Disease-induced }\DIFaddend mortality is modeled \DIFdelbegin \DIFdel{similar }\DIFdelend \DIFaddbegin \DIFadd{similarly }\DIFaddend to model 1 and 2. However, as \DIFaddbegin \DIFadd{the }\DIFaddend single state does not exist in model 3 and 4, individuals that \DIFdelbegin \DIFdel{has left their }\DIFdelend \DIFaddbegin \DIFadd{leave }\DIFaddend partnerships due to death of their partners \DIFdelbegin \DIFdel{enter temporary compartments and form partners }\DIFdelend \DIFaddbegin \DIFadd{form partnerships }\DIFaddend instantly:
...
Following \DIFaddbegin \DIFadd{Shirreff \etal }\DIFaddend \cite{shirreff_transmission_2011}, Model 5 is an implicit instantaneous partnership formation model that uses an adjusted transmission rate, \DIFdelbegin \DIFdel{$\beta'$}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\ast$}\DIFaddend , that is derived from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hollingsworth_hiv1_2008}}%DIFAUXCMD
...
We follow Champredon \etal's result to calculate the initial distribution of infected individuals. For model 1 and 2, we have disease equilibrium state of $S^* = \frac{c}{c + \rho}$ and $SS^* = \frac{1-S^*}{2}$. We let $\epsilon = 10^{-4}$, which is the total number of infected individuals in the beginning of simulation and $D$ be the vector such that $D_i$ represent the proportion of individuals with \Lspvl\ of $i$. \DIFdelbegin \DIFdel{$Y_i$ }\DIFdelend \DIFaddbegin \DIFadd{$D_i$ }\DIFaddend is taken from \DIFdelbegin \DIFdel{normal }\DIFdelend \DIFaddbegin \DIFadd{the Gaussian }\DIFaddend distribution with mean 3 and \DIFaddbegin \DIFadd{standard deviation 0.2 and }\DIFaddend is normalized so that $\sum_i D_i = 1$. Then, we have the following initial distribution of each states:
