...
%DIF ADD plos_latex.tex       Mon Oct 24 11:59:34 2016
% Template for PLoS
...
complexity as used by Fraser }\emph{\DIFadd{et al.}} 
...
(\DIFdelbegin \DIFdel{we refer to SPVL as }\DIFdelend \DIFaddbegin \DIFadd{hereafter }\DIFaddend ``virulence'' \DIFdelbegin \DIFdel{hereafter}\DIFdelend \DIFaddbegin \DIFadd{will refer either to the SPVL, or to 
the rate of progression to AIDS; these two quantities are
deterministically linked in the model}\DIFaddend ).
In contrast to Shirreff \etal, we use a single-stage disease model instead of accounting explicitly for progression through the three main stages of HIV infection (primary, asymptomatic, and disease), and we use a simple exponentially distributed infectious period instead of a more realistic Weibull-distributed infectious period\DIFaddbegin \DIFadd{; we show below that our results are not overly
sensitive to this simplification}\DIFaddend . We account for varying transmission rates and durations of each disease stage by summing the durations of three stages (again based on Shirreff \etal's model) and taking the duration-weighted average of transmission rates of three stages. Thus the within-couple transmission rate, $\beta$, for our models is given by:
\begin{equation}
...
}\DIFdelend \DIFaddbegin \DIFadd{(see Appendix). Simplifying the model
...
(see Appendix). We assume random activity-weighted mixing between risk groups \mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{: see Appendix).
}\DIFaddend 
...
\DIFdelbegin \DIFdel{The one completely new }\DIFdelend \DIFaddbegin \DIFadd{One }\DIFaddend parameter in our model, the ratio of uncoupled to within-couple transmission $c_u/c_w$, is needed to more flexibly contrast uncoupled and extra-couple transmission dynamics within multi-strain models (Appendix S1);
...
%DIF > %  BMB: I don't think we need to go into detail about variance vs CV
...
of progression time }\DIFdelend \DIFaddbegin \DIFadd{see Appendix for 
...
Single individuals acquire partners at per-person rate $\rho$. Partnership formation \DIFdelbegin \DIFdel{rate }\DIFdelend \DIFaddbegin \DIFadd{rates }\DIFaddend for $S$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $I_i$ are \DIFdelbegin \DIFdel{$- \rho S$ and $-\rho I_i$}\DIFdelend \DIFaddbegin \DIFadd{$\rho S$ and $\rho I_i$}\DIFaddend , respectively. We follow Champredon \etal \DIFdelbegin \DIFdel{'s results and assume }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{champredon_hiv_2013} }%DIFAUXCMD
in assuming }\DIFaddend that single individuals are distributed into coupled states with pair-formation (PF) rates as follows:
...
Partnerships dissolve at per-partnership rate $c$: the dissolution rates for $SS$, $SI_i$, and $II_{ij}$ pairs are $c SS$, $c SI_i$, and $c II_{ij}$ respectively. Unlike \DIFdelbegin \DIFdel{single strain }\DIFdelend \DIFaddbegin \DIFadd{a single-strain }\DIFaddend model, where both individuals leaving the $II$ partnership would enter $I$, we have to account for strains \DIFaddbegin \DIFadd{with }\DIFaddend which the individuals in concordant partnership are infected \DIFdelbegin \DIFdel{with }\DIFdelend (i.e. both partners in $II_{ii}$ enter $I_i$ whereas one partner in $II_{ij}$ enters the $I_i$ compartment while the other enters $I_j$). Thus, coupled individuals are distributed into single states through partnership dissolution (DS) \DIFaddbegin \DIFadd{rates}\DIFaddend :
...
Within-couple transmission (WT) occurs in both models. An infected partner in $SI$ partnership transmits virus to a susceptible partner \DIFdelbegin \DIFdel{, and partnership state becomes $II$}\DIFdelend \DIFaddbegin \DIFadd{at per-partnership rate $\beta$}\DIFaddend : $\WT(SI_i) = - \beta_i SI_i$. Since we assume that mutation occurs, $II_{ij}$ \DIFaddbegin \DIFadd{pairs}\DIFaddend , where $i \neq j$, can be formed from \DIFdelbegin \DIFdel{both }\DIFdelend \DIFaddbegin \DIFadd{either }\DIFaddend $SI_i$ \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend $SI_j$ \DIFdelbegin \DIFdel{partnership}\DIFdelend \DIFaddbegin \DIFadd{partnerships}\DIFaddend : $\WT(II_{ij}) = M_{ij} \beta_i SI_i + M_{ji} \beta_j SI_j$. On the other hand, $II_{ii}$ can only be formed from an $SI_i$ partnership: $\WT(II_{ii}) = M_{ii} \beta_i SI_i$. Using the Kronecker delta notation, we obtain the following set of equations \DIFdelbegin \DIFdel{that describes }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend within-couple transmission dynamics:
...
Single susceptible individuals become infected \DIFdelbegin \DIFdel{and enter single infected compartment at the total rate of $\sum_k P_k U_k S$}\DIFdelend \DIFaddbegin \DIFadd{through uncoupled contact at per-person rate $\sum_k P_k U_k$ and enter the single infected state}\DIFaddend . Through mutation, newly infected individuals are distributed into single infected compartments with different strains: $\ET(I_i) = \sum_k M_{ki} P_k U_k S$. Either partner in an $SS$ partnership can \DIFdelbegin \DIFdel{be infected , with the partnership state becoming }\DIFdelend \DIFaddbegin \DIFadd{become infected at per-person rate $\sum_k P_k E_k$, and partnership state changes to an }\DIFaddend $SI$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{partnership }\DIFaddend at the total rate of $\sum_i 2 P_i E_i SS$. The formation of $SI_i$ partnerships is similar to the process through which single susceptible individuals are distributed into single infected compartments: $\ET(SI_i) = \sum_k 2 M_{ki} P_k E_k SS$. Lastly, the susceptible partner in an $SI$ partnership can \DIFdelbegin \DIFdel{be infected due to uncoupled/}\DIFdelend \DIFaddbegin \DIFadd{become infected from }\DIFaddend extra-couple contacts \DIFdelbegin \DIFdel{and partnership can change }\DIFdelend \DIFaddbegin \DIFadd{at a per-person rate of $\sum_k P_k E_k$, so that the partnership changes }\DIFaddend to an $II$ partnership. As in the previous cases, $SI_i$ partnerships are lost at a rate of $\sum_k P_k E_k SI_i$. The mutation process is similar to that of within-couple transmission. The only difference is that the \Lspvl\ of a newly infected partner is not determined by its social partner but from an extra-couple partner (i.e. the term $P_i$): $\ET(II_{ij}) = (\frac{1}{2})^{\delta_{ij}}(\sum_k (M_{kj} P_k E_k SI_i + M_{ki} P_k E_k SI_j))$. Combining these equations we get the following set of equations that describe \DIFdelbegin \DIFdel{all the }\DIFdelend \DIFaddbegin \DIFadd{the complete }\DIFaddend transmission dynamics:
...
Model 3 and 4 share the within-couple transmission term with model 1 and 2. Since there is no single (uncoupled) state, only extra-couple transmission \DIFdelbegin \DIFdel{exists}\DIFdelend \DIFaddbegin \DIFadd{occurs}\DIFaddend :

\begin{equation}
P_i = \frac{r_e (SI_i + \sum_k (II_{ik} + \delta_{ik} II_{ik}))}{r_e(2 SS + \sum_k 2 SI_k + \sum_l \sum_k (2^\delta_{kl} II_{lk}) )}.
\end{equation}
Movement from $SS$ state to $SI$ state and $SI$ to $SS$ is modeled through the same equation that is used in \DIFdelbegin \DIFdel{model }\DIFdelend \DIFaddbegin \DIFadd{models }\DIFaddend 1 and 2.

\subsubsection*{Instantaneous-switching models: Disease induced mortality}

\DIFdelbegin \DIFdel{Disease induced }\DIFdelend \DIFaddbegin \DIFadd{Disease-induced }\DIFaddend mortality is modeled \DIFdelbegin \DIFdel{similar }\DIFdelend \DIFaddbegin \DIFadd{similarly }\DIFaddend to model 1 and 2. However, as \DIFaddbegin \DIFadd{the }\DIFaddend single state does not exist in model 3 and 4, individuals that \DIFdelbegin \DIFdel{has left their }\DIFdelend \DIFaddbegin \DIFadd{leave }\DIFaddend partnerships due to death of their partners enter temporary compartments and form \DIFdelbegin \DIFdel{partners }\DIFdelend \DIFaddbegin \DIFadd{partnerships }\DIFaddend instantly:
...
Following \cite{shirreff_transmission_2011}, Model 5 is an implicit instantaneous partnership formation model that uses an adjusted transmission rate, \DIFdelbegin \DIFdel{$\beta'$}\DIFdelend \DIFaddbegin \DIFadd{$\beta^\ast$}\DIFaddend , that is derived from \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hollingsworth_hiv1_2008}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Hollingsworth \etal}\DIFaddend 's approximate basic reproduction number \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{hollingsworth_hiv1_2008}}%DIFAUXCMD
}\DIFaddend :
...
Model 6 is a \DIFdelbegin \DIFdel{random mixing }\DIFdelend \DIFaddbegin \DIFadd{random-mixing }\DIFaddend model. It is modeled in a same way as model 5 without the adjusted transmission rate:
...
\DIFadd{For dissolution process, an individual in risk group $i$ leaves its partnership at a rate $\varphi_i c$. If a partnership is formed between two individuals from a different risk group, the rates at which they leave the partnership differ. We resolve this conflict by assuming that a partnership dissolution rate of a couple is equal to the average of that of two partners. Therefore, $XY_{ij, kl}$ dissolve at per-partnership rate $(\varphi_k + \varphi_l)/2 c$, and both $X_{i,k}$ and $Y_{j,l}$ partners return to single state at the same rate.
}

\subsubsection*{\DIFadd{Heterogeneous models: Infection dynamics}}

\DIFadd{Since we assume that the rate of transmission per partnership stays constant across different risk groups, the within-couple infection process is similar to other models:
}
...
We follow Champredon \etal's result to calculate the initial distribution of infected individuals. For model 1 and 2, we have disease equilibrium state of $S^* = \frac{c}{c + \rho}$ and $SS^* = \frac{1-S^*}{2}$. We let $\epsilon = 10^{-4}$, which is the total number of infected individuals in the beginning of simulation and $D$ be the vector such that $D_i$ represent the proportion of individuals with \Lspvl\ of $i$. $Y_i$ is taken from \DIFdelbegin \DIFdel{normal }\DIFdelend \DIFaddbegin \DIFadd{the Normal }\DIFaddend distribution with mean 3 and is normalized so that $\sum_i D_i = 1$. Then, we have the following initial distribution of each states:
...
\end{aligned}
\end{equation}
Since model 3 and 4 do not have single \DIFdelbegin \DIFdel{state}\DIFdelend \DIFaddbegin \DIFadd{states}\DIFaddend , $SS^*=1$ at the disease free equilibrium and the initial distribution becomes\DIFdelbegin \DIFdel{as follows}\DIFdelend :
...
}\DIFdelend \DIFaddbegin \DIFadd{This section presents alternate versions of
the figures from the main text showing time dynamics and summary
statistics in terms of }\Lspvl\DIFadd{\ and per-year transmission probability
...
\DIFdel{HIV}%DIFDELCMD < }%%%
\DIFdel{-1 transmissionper coital act, by stage of }%DIFDELCMD < {%%%
\DIFdel{HIV}%DIFDELCMD < }%%%
\DIFdel{-1
