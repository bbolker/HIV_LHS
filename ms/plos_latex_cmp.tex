...
%DIF ADD plos_latex.tex       Sun Oct 23 14:04:44 2016
% Template for PLoS
...
rate }\DIFdelend \DIFaddbegin \DIFadd{where reproduction
...
--- postulates that
the between-host birth and death rate are both linked to the rate at which
...
\DIFaddbegin \DIFadd{Two parameters, mean and the squared coefficient of variation (CV) of number of non-cohabiting partners, are sampled for heterogeneity in sexual activity. We decided to use squared CV instead of variance as variance measures is often unreliable \todo{CITE? You mentioned that variance is unreliable and we should use CV but we didn't talk about it very much...}. To allow for a wide range of uncertainty, range for the mean number of non-cohabiting partners was taken from unmarried men as that was the group with the largest variability \mbox{%DIFAUXCMD
\cite{omori2015dynamics}}%DIFAUXCMD
. However, we decided to use 0.01 - 100 as the sampling range for squared CV despite the wider CV range given by Omori \etal \mbox{%DIFAUXCMD
\cite{omori2015dynamics} }%DIFAUXCMD
(0 - 20.324). Although married female have largest variability for CV, the range of the mean is narrow and small, which might explain for such large variability. Assessing ranges of CV of other groups, we concluded that 0.01 - 100 is both reasonable and wide enough to cover uncertainties in parameters. For simplicity, we assume that the number of non-cohabiting partner follows a gamma distribution.
}
