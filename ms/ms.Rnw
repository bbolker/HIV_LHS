...
\end{equation}
We have experimented with varying degrees of discretization in the strain distribution (i.e., values of $n$); in our model runs comparing results with \cite{shirreff_transmission_2011} (Figure~\ref{fig:panel3}) we use $n=51$ (i.e. a bin width of 0.05 \Lspvl\ for $\alpha$), but we find only small differences when reducing $n$ to 21 (bin width 0.25 \Lspvl), which we use for all other simulations.
...
\todo{better explanation of why increasing $\beta$/$D$ and
then scaling $r$ should lead to lower $\rzero$?}
...
\citep{wawer2005rates,hollingsworth_hiv1_2008}
...
\section{To do}
\begin{itemize}
\item use PLoS template for ms
\item check on code/documentation on repo
...
\item figures:
  \begin{itemize}
  \item fig 1. tweak label position in (a)
  \item fig 3. add units to strip labels?
  \item fig 4. fix axis labels; squash panels together
  \item fig 5. fix strip labels for summary statistics; fix x-axis labels
  \item create fig A1 (scaled sensitivity plot) for supp materials
  \end{itemize}
\end{itemize}
