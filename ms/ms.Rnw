...
successfully applied in several important cases \citep{Dwyer+1990}
\todo{others? de Roode and Altizer? maybe \cite{mackinnon1999genetic},
  \cite{jensen2006empirical}, and \cite{de2008virulence}?}. One
particularly interesting application of these ideas is the work by
Fraser et al. showing that HIV appears to satisfy the prerequisites of
...
\begin{itemize}
\item Since we assume that SPVL is determined by genoyype, we should probably mention papers like this:\cite{alizon2010phylogenetic}
\item Do we need to mention things like heritability of SPVL or do we not have to mention it since it's already included in Shirreff et al's paper and our models are based on their model? It's an important assumption and maybe we need to cite source..?
\end{itemize}
...
of mean $\log_{10}$ SPVL of 3 and epidemic size of $10^{-4}$ (refer to appendix). After each run, initial exponential growth rate is calculated. Then, parameters are scaled so that the initial exponential growth rate is scaled to 0.04, which is approximately equal to that of Shirreff et al's model.
