...
\todo{BMB: add to this description}
In disease modeling, one of the most important parameter is the initial exponential growth rate. As it is one of the few parameters that can be directly observed, many modelers try to match the initial exponential growth rate to correctly simulate the epidemic. Furthermore, evolution of virulence is sensitive to the initial growth rate. Thus, we scale a parameter so that every run has an equal exponential rate of growth rate.
...
<<panel3,fig.cap="Time plot (3-panel plot): baseline dynamics, illustrating summary statistics (peak time, eq virulence, length of time until peak virulence). Full Shirreff model, effects of $I(0)$, $\\alpha(0)$, $r$",dev="tikz",fig.width=10>>=
for (i in c("","2","3")) 
...
g1 <- ggplot(df_tot,aes(x=Time,y=Mean.VL,colour=run))+
    geom_line()+labs(x="time",y="mean virulence ($\\log_{10}$ SPVL)")
###
...
g2.y <- g2 + scale_y_continuous(limits = limits, breaks = breaks)+
    scale_colour_discrete(name="$I(0)$")
g3.y <- g3 + scale_y_continuous(limits = limits, breaks = breaks)+
        scale_colour_discrete(name="$\\alpha(0)$")
grid.arrange(g1.y, g2.y, g3.y, nrow=1)
@ 

<<virtraj,fig.cap="Envelopes of virulence trajectories under all models">>=
...
<<unidist,fig.cap="Univariate distributions of summary statistics",fig.width=6,fig.height=6>>=
sL <- transform(sum_list[["sum_mat"]],rel_peak=peak_vir/eq_vir)
mL <- melt(sL,id.vars=c("model","run"))
ggplot(mL,aes(value,model,fill=model))+
    geom_violinh(scale="area")+
...
Figure to-do:
\begin{itemize}
\item fig 1. move legends into panels: tweak size? remove redundant y axes?
\item fig 3. add random-mixing model
\item fig 4. re-order, figure out how to tweak random density width
\end{itemize}
