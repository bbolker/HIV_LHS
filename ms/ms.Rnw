...
<<setup,echo=FALSE>>=
library("knitr")
opts_chunk$set(echo=FALSE,error=FALSE,fig.width=4,fig.height=4) #
@
...
@ 
\begin{itemize}
\item Time plot (3-panel plot): baseline dynamics, illustrating summary statistics (peak time, eq virulence, length of time until peak virulence). Full Shirreff model, effects of $I(0)$, $\alpha(0)$, $r$
\item Virulence trajectories under all models
