...
pairform+epc model. When individuals spend time uncoupled between
...
mL3 <- subset(mL2,
       !((model=="implicit" &
         ...
         (model=="random" &
          LHSvar %in% c("rho_base","c_u_ratio","c_e_ratio","c_mean_base")) |
         (model=="instswitch" &
          LHSvar %in% c("rho_base","c_u_ratio","c_e_ratio")) |
...
which is the only model to which it applies\todo{is that right??}.
...
modeling. In addition to following \champr\ by doing Latin hypercube
...
\todo{do we know what $r$ Champredon et al. get??}
