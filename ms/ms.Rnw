...
\todo{shireff says that the interval between two strains is 0.05 but uses 0.1 in the code... Do we want to do a simulation with 101 strains or is it OK to keep it as 51? It wouldn't make any but I guess it's better to match what's written in the paper? Shirreff simulation that was used in figure 1 was done by using 51 strains.}
...
sets for these models lead to low equilibrium virulence ($\approx 2.5 \log_{10}$ SPVL) \todo{period? semicolon?}
...
pairform+epc model \todo{might be better to call it something else?}. When individuals spend time uncoupled between
...
which is the only model to which it applies\todo{is that right?? Yes}.
...
modeling. In addition to following \champr\ \todo{cite?} by doing Latin hypercube
...
growth rates ($\approx 0.4~\textrm{year}^{-1}$) \todo{0.04 not 0.4?}
