...
<<tradeoff_fig,echo=FALSE,fig.height=14,fig.width=30,out.width="0.95\\textwidth",dev="tikz",cache=TRUE>>=
print(gg_tradeoff)
@ 

\begin{itemize}
\item \textbf{tradeoff theory}: virulence evolution mediated by transmission-vs-clearance tradeoff
...
\begin{block}{Basic eco-evolutionary dynamics}
...
<<univ,cache=TRUE,dev="tikz",fig.width=45,fig.height=40,out.width="0.95\\textwidth">>=
