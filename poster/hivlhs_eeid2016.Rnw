...
\usepackage[orientation=portrait,size=custom,width=71,height=102,scale=1.2,debug]{beamerposter}
%## size=a0,
    ...
    \begin{beamercolorbox}[center,wd=\textwidth]{postercolumn}
      \begin{minipage}[T]{.95\textwidth}
        \parbox[t][\topcolumnheight]{\textwidth}{ 
          \vspace{-4.5cm} % shouldn't need this??
...
\begin{beamercolorbox}[center,wd=\textwidth]{postercolumn}
  \begin{minipage}[T]{.95\textwidth} % tweaks the width, makes a new \textwidth
    \parbox[t][26cm]{\textwidth}{ % must be some better way to set

\begin{block}{Sensitivity}
\includegraphics[width=0.9\textwidth]{figure/sens-1.png}
<<sens,dev="png",echo=FALSE,fig.width=20,fig.height=10,cache=TRUE,out.width="0.95\\textwidth",eval=FALSE>>=
mL3 <- subset(mL2,
       !((model=="implicit" &
...
@
\begin{itemize}
\item {\small $\beta_{P,D}$: early/late-stage transmission probability; $D_{P,D}$: early/late-stage duration; $c$: partnership dissolution rate; $c_e/c_w$: extra- vs within-pair contact ratio; $\rho$: partnership formation rate; $c_u/c_w$: uncoupled contact rate ratio}
\end{itemize}
\end{block}
}
        \end{minipage}
...
\item Variation among models (model structure) $\gtrsim$ variation within models (parameter uncertainty)
