...
<<setup3, echo=FALSE,dev="tikz",cache=TRUE>>=
source("../R/hivFuns.R")
source("../R/LHSparms3.R")
Lspvl <- "$\\log_{10}~\\textrm{SPVL}$"
pars <- expand(ltab_mean)
dd <-with(pars,
...
neglect important epidemiological processes, such as the formation and
dissolution of sexual partnerships. We
compared \textbf{models with a range of complexity} in their
representation of partnership dynamics and extra-partnership contact.
Simple random-mixing actually gave the closest match to the most realistic models; extra-pair contact washed out the effects
...
<<tradeoff_fig,echo=FALSE,fig.width=10>>=
print(gg_tradeoff)
@ 
  \begin{itemize}
    \item \textbf{tradeoff theory}: virulence evolution
      mediated by transmission-vs-clearance tradeoff
    \item still debated \cite{EbertBull2003,alizon_adaptive_2015}
    \item HIV \cite{fraser_virulence_2014}: \textbf{set-point viral load}
      correlated with transmission probability, rate of progression to AIDS
    \item data from Rakai, Uganda
...
\item \textbf{extra-pair contact} (\textbf{epc}): present or absent?
...
Slightly simplified disease model (single stage only;
genetic but not phenotypic variation)
...
\begin{block}{Basic evolutionary dynamics}

\vspace{-1cm}

<<virdyn,cache=TRUE,dev="tikz",fig.width=25,fig.height=30>>=
...
<<univ,cache=TRUE,dev="tikz",fig.width=45,fig.height=30>>=
...
<<ggpairs,echo=FALSE,fig.width=25,fig.height=25>>=
sL2 <- sL
      ...
      \begin{beamercolorbox}[center,wd=\textwidth]{postercolumn}
        \begin{minipage}[T]{.95\textwidth} % tweaks the width, makes a new \textwidth
          \parbox[t][\columnheight]{\textwidth}{ % must be some better way to set the the height, width and textwidth simultaneously
            ...
            \begin{block}{Sensivity}
<<sens,dev="png",echo=FALSE>>=
mL3 <- subset(mL2,
       !((model=="implicit" &
