    ...
    g <- function(t,yini,parameters) {
        with(as.list(c(yini,pp)), 
        {
            ...
            list(c(dS, dI, dSS, dSI, dII), 
            		 ##tot_I = tot_I, mean_V = tot_V/tot_I
            		 mort.SI.out)
        })
    }
}
...
full_model_rcpp <- function(t, yini, parameters){
	return(list(unlist(g2(t, yini, parameters))))
}

pp <- expand(HIVpars.mean)

tvec <- seq(1, 30, 0.5)

r_rcpp <- lsoda(unlist(yini2), func = full_model_rcpp, parms = pp, times = tvec)
...
microbenchmark(
tmpa = c(unname(g2(0, unlist(yini4), expand(HIVpars.mean))$dSI)),
tmpb = c(unname(model2(expand(HIVpars.mean), infection = TRUE, mortality = TRUE)(0, unlist(yini4), expand(HIVpars.mean))[[1]][76:264]))
)
