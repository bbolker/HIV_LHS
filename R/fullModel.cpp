...
//[[Rcpp::export]]
List rcppFull(double t, NumericVector yini, List parameters) {
	...
	//alpha
	NumericVector alpha2(nalpha * nrisk);
  ...
  //	tmp.vec <- c(1:n.alpha) + (i-1) * n.alpha
	...
	NumericVector inf(nrisk * nalpha);
	
	for(i = 0; i < nrisk * nalpha; i ++){
		inf[i] = I[i] + sum(SI(_,i)) + sum(II_adj(_,i));
	}
	
	double tot_I = sum(inf);
	double mean_V = sum(alpha2 * inf)/tot_I;
	...
	return Rcpp::List::create(Rcpp::Named("dS") = dS,
                           ...
                           Rcpp::Named("dSI") = dSI,
                           Rcpp::Named("dII") = dII,
                           Rcpp::Named("tot_I") = tot_I,
                           Rcpp::Named("mean_V") = mean_V);
	
}

//[[Rcpp::export]]
List rcppPartner(double t, NumericVector yini, List parameters) {
	...
	dS = - rho2 * S;
	for(i = 0; i < nrisk; i ++){
		dS[i] += sum(cc_mat(_,i) * SS_adj(_,i)) + sum(d_SI(i,_));
	}
	...
	dI = - rho2I * I;
	for(i = 0; i < nrisk * nalpha; i ++){
		dI[i] += sum(d_SI(_,i)) + sum(cc_matII(_,i) * II_adj(_,i));
	}
...
//[[Rcpp::export]]
List rcppFull_noepc(double t, NumericVector yini, List parameters) {
	...
	dS = - rho2 * S;
	for(i = 0; i < nrisk; i ++){
		dS[i] += sum(cc_mat(_,i) * SS_adj(_,i)) + sum(d_SI(i,_)) + mortIsus[i] + mortSIsus[i] + mortIIsus[i];
	}
	...
	dI = - rho2I * I;
	for(i = 0; i < nrisk * nalpha; i ++){
		dI[i] += sum(d_SI(_,i)) + sum(cc_matII(_,i) * II_adj(_,i)) - mortIout[i] + mortIII[i];
	}
	...
	for(i = 0; i < nrisk; i ++){
		for(j = 0; j < nrisk * nalpha; j ++){
			dSI(i, j) = fSI(i, j) - d_SI(i,j) - infSIfrom(i,j) - mortSIout(i,j);
		}
	}
	...
	for(i = 0; i < nrisk * nalpha; i ++){
		for(j = 0; j < nrisk * nalpha; j ++){
			dII(i,j) = fII(i,j) - cc_matII(i,j) * II(i,j) + infSIto(i,j) - mortIIout(i,j);
		}
	}
